<script>
  ...
  let answers = {
    ...
    tenYearCapacity: 8e9, // Default to humanity's level
    tenYearCapacityLog: 10, // Default to 10^10 = 10 billion (slightly above humanity)
  };
  ...
  const questions = [
    ...
    {
      id: 'tenYearCapacity',
      text: 'Ten years after the first digital mind is created, what do you expect the welfare capacity to be?',
      type: 'ten-year-capacity',
      description: 'This represents the expected welfare capacity after 10 years of development',
    },
    {
      id: 'moralProgress',
    ...
    const scenario = {
      ...
      moralConsideration: {
        type: answers.moralProgress === 'early' ? 'coupled' :
              answers.moralProgress === 'gradual' ? 'delayed' :
              answers.moralProgress === 'late' ? 'uncoupled' : 'decoupling',
        level: answers.moralProgress === 'early' ? 0.8 :
               answers.moralProgress === 'gradual' ? 0.5 :
               answers.moralProgress === 'late' ? 0.3 : 0.3
      },
      expectedValue: {
        credence: answers.credence,
        initialCapacity: answers.initialCapacity,
        maxCapacity: answers.maxCapacity,
        capacityAfterTenYears: answers.tenYearCapacity
      }
    };
...
>
  <div class="bg-gray-800 rounded-xl p-8 max-w-2xl w-full mx-4 shadow-2xl border border-gray-700">
    ...
    {:else if questions[currentQuestion].type === 'ten-year-capacity'}
      <div class="space-y-4">
        <p class="text-gray-400 text-sm mb-4">{questions[currentQuestion].description}</p>
        <div class="flex flex-col gap-4">
          <input 
            ...
            step="0.1"
            class="w-full accent-purple-500"
            bind:value={answers.tenYearCapacityLog}
            on:input={() => answers.tenYearCapacity = Math.pow(10, answers.tenYearCapacityLog)}
          />
          <div class="flex justify-between items-center">
            <span class="text-gray-400 text-sm">4B</span>
            <span class="text-gray-100 font-mono text-lg">
              {formatNumber(answers.tenYearCapacity)}
            </span>
            <span class="text-gray-400 text-sm">80T</span>
          </div>
          <button
            class="w-full p-4 text-center rounded-lg border border-gray-700 hover:border-purple-500 
                   bg-gray-900 hover:bg-gray-800 transition-all duration-200 text-gray-100"
